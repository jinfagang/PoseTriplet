...
policy_net = PolicyGaussian(
    MLP(state_dim, cfg.policy_hsize, cfg.policy_htype),
    action_dim,
    log_std=cfg.log_std,
    fix_std=cfg.fix_std,
)
...
if cfg.policy_optimizer == "Adam":
    optimizer_policy = torch.optim.Adam(
        policy_net.parameters(), lr=cfg.policy_lr, weight_decay=cfg.policy_weightdecay
    )
else:
...
if cfg.value_optimizer == "Adam":
    optimizer_value = torch.optim.Adam(
        value_net.parameters(), lr=cfg.value_lr, weight_decay=cfg.value_weightdecay
    )
else:
...
agent = AgentPPO(
    ...
    mean_action=args.render and not args.show_noise,
    render=args.render,
    ...
    policy_grad_clip=[(policy_net.parameters(), 40)],
...
def main_loop():
    if args.render:
        ...
        for i_iter in range(args.iter, cfg.max_iter_num):
            ...
            print('finished update params on ', batch)
            t1 = time.time()
            ...
            logger.info(
                ...
                "\texpert_R_range ({:.4f}, {:.4f})\teps_len {:.2f}".format(
                    i_iter,
                    log.sample_time,
                    t1 - t0,
                    get_eta_str(i_iter, cfg.max_iter_num, t1 - t0 + log.sample_time),
                    log.avg_c_reward,
                    np.array2string(
                        c_info, formatter={"all": lambda x: "%.4f" % x}, separator=","
                    ),
                    log.min_c_reward,
                    log.max_c_reward,
                    log.avg_episode_len,
            ...
            for i in range(c_info.shape[0]):
                ...
                tb_logger.add_scalar(
                    "eps_reward_%d" % i, log.avg_episode_c_info[i], i_iter
                )

            if (
                cfg.save_model_interval > 0
                and (i_iter + 1) % cfg.save_model_interval == 0
            ):
                tb_logger.flush()
