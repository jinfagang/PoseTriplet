...
def bbox_iou(box1, box2):
    ...
    inter_area = torch.max(
        inter_rect_x2 - inter_rect_x1 + 1, torch.zeros(inter_rect_x2.shape).to(box1.device)
    ) * torch.max(inter_rect_y2 - inter_rect_y1 + 1, torch.zeros(inter_rect_x2.shape).to(box2.device))
