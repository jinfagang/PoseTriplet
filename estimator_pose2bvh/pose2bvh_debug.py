...
def write_standard_bvh(outbvhfilepath, prediction3dpoint):
    ...
    bvhfileName = os.path.join(
        dir_name, video_name, "bvh", "{}_h36m.bvh".format(video_name)
    )
...
if __name__ == "__main__":
    ...
    prediction = np.load(
        "bvh_skeleton/test_data/h36m_take_599_predicted_3d_wpos.npy", allow_pickle=True
    )[690:710]
    prediction = np.load(
        "/Users/jintian/dev/codes/ai/metaverse/mimicry/vendor/PoseTriplet/estimator_inference/wild_eval/pred3D_pose/bilibili-clip/kunkun_clip_pred3D.pkl",
        allow_pickle=True,
    )['result']
