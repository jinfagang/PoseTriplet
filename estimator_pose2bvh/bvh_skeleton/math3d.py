...
def dcm_from_axis(x_dir, y_dir, z_dir, order):
    ...
    axis[order[0]] = normalize(axis[order[0]])  # 固定第一个的axis
    test1 = name[(idx1 + 1) % 3]  # debug
    test2 = name[(idx1 + 2) % 3]  # debug
    axis[order[1]] = normalize(np.cross(
        axis[name[(idx1 + 1) % 3]], axis[name[(idx1 + 2) % 3]]
    ))  # 第二个用已知的两个cross,
    # 改进: 刚才是手的时候, shoulder的upper arm, upper arm 刚好和shoulder是90度,
    # cross的结果会是很大的值, 所以这块不会出错, 大腿小腿用的是可能平行, 误差大.
    ...
    # dcm[:, -1:] = dcm[:, -1:] * -1  # 左右手坐标系转换测试.
    # 检查dcm转左右手成功没,
    # 检查quat是不是一样的,
    # 检查euler是不是一样的.

    # dcm = np.array([
    #     [dcm[0,0], dcm[0, 2], dcm[0, 1]],
    #     [dcm[2,0], dcm[2, 2], dcm[2, 1]],
    #     [dcm[1,0], dcm[1, 2], dcm[1, 1]],
    #                 ])
    return dcm
